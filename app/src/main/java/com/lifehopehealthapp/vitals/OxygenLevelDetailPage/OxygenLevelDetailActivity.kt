package com.lifehopehealthapp.vitals.OxygenLevelDetailPage
...
import com.lifehopehealthapp.databinding.ActivityOxygenLevelDetailpageBinding
...
class OxygenLevelDetailActivity :
    BaseActivity<OxygenLevelDetailViewModel, OxygenLevelDetailModel>() {

    private var image: String? = ""
    private var vitalID: String? = ""
    private var name: String? = ""
    private var lastValue: String? = ""
    private var lastDate: String? = "1"
    ...
    private lateinit var binding: ActivityOxygenLevelDetailpageBinding

    override fun getViewModel() = OxygenLevelDetailViewModel::class.java

    override fun getActivityRepository() = OxygenLevelDetailModel(
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        binding = ActivityOxygenLevelDetailpageBinding.inflate(layoutInflater)
...
*/
        ...
        binding.buttonAddVitals.setOnClickListener {
            intent = Intent(this@OxygenLevelDetailActivity, AddVitalsActivity::class.java)
        ...
        binding.tabLayout.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab) {
                ...
                (binding.tabLayout as TabLayout).setTabTextColors(
                    ContextCompat.getColor(this@OxygenLevelDetailActivity, R.color.white),
                    ContextCompat.getColor(this@OxygenLevelDetailActivity, R.color.purple_500)
                )
            }
                ...
                if (s.toString().contains("M") || s.toString().contains("D") || s.toString()
                    ...
                    if (length == 10) {
                        if (Utils.compareWithCurrentDate(current) > 0) {
                            Utils.showToast(
                                this@OxygenLevelDetailActivity,
    ...
    private fun apiCall() {
        if (Utils.isNetworkAvailable(this)) {
            ...
            json = gson.toJson(data)
            Log.e("Model---->", json.toString())
            if (binding.oxygenChart.data != null) {
                binding.oxygenChart.getData().clearValues()
                binding.oxygenChart.clear()
            }
                ...
                when (it) {
                    is Resource.Success -> {
                        ...
                        if (it.value.getStatusCode() == 200) {
                            ...
                            mColor = it.value.getValue()!!.data!!
                                .getBarColorSchemes() as ArrayList<BarColorScheme>
                            Log.e("mValue", mDate.size.toString())
                            if (mValue.size == 0) {
                                binding.mProgressBar.isVisible = false
                                ExitingDate = ""
    ...
    private fun initGraphValue(
        ...
        timeStamp: ArrayList<BarTimestamp>,
        mColor: ArrayList<BarColorScheme>
    ) {
        binding.oxygenChart.setDrawBarShadow(false)
        binding.oxygenChart.setDrawValueAboveBar(true)
        binding.oxygenChart.getDescription().setEnabled(false)
        binding.oxygenChart.setMaxVisibleValueCount(60)
        binding.oxygenChart.setPinchZoom(false)
        binding.oxygenChart.isDoubleTapToZoomEnabled = false
        binding.oxygenChart.setDrawGridBackground(false)
        binding.oxygenChart.getViewPortHandler().setMaximumScaleX(15f)
        binding.oxygenChart.getViewPortHandler().setMaximumScaleY(2f)
        binding.oxygenChart.setVisibleXRangeMaximum(4f)
        binding.oxygenChart.setVisibleXRangeMinimum(4f)

        val xl: XAxis = binding.oxygenChart.getXAxis()
        ...
        //xl.valueFormatter = IAxisValueFormatter { value, axis -> value.roundToInt().toString() }


        val yl: YAxis = binding.oxygenChart.getAxisLeft()
        yl.isEnabled = false

        val custom: IAxisValueFormatter = MyAxisValueFormatter(chartMetrics)
        val yr: YAxis = binding.oxygenChart.getAxisRight()
        yr.valueFormatter = custom
        yr.axisMinimum = graphMin!!.toFloat()
        yr.axisMaximum = graphMax!!.toFloat()
        yl.axisMinimum = graphMin!!.toFloat()
        yl.axisMaximum = graphMax!!.toFloat()


        binding.oxygenChart.animateY(1500)

        val l: Legend = binding.oxygenChart.getLegend()
        ...
        setData(mValue, xl, timeStamp, mColor)

        binding.oxygenChart.invalidate()
    }
    ...
    private fun setData(
        ...
        xl.valueFormatter = object : IAxisValueFormatter {
            ...
            override fun getFormattedValue(value: Float, axis: AxisBase): String {
                ...
                return if (value.toInt() < mDateValue.size) mFormat.format(Date(timeStamp.toLong() * 1000)) else "0"
            }
        }
        ...
        val set1: BarDataSet
        if (binding.oxygenChart.getData() != null &&
            binding.oxygenChart.getData().getDataSetCount() > 0
        ) {
            set1 = binding.oxygenChart.getData().getDataSetByIndex(0) as BarDataSet
            set1.values = values
            binding.oxygenChart.getData().notifyDataChanged()
            binding.oxygenChart.notifyDataSetChanged()
        } else {
            set1 = BarDataSet(values, "Oxygen Level(SPO2)")
            ...
            data.barWidth = 0.3f
            binding.oxygenChart.setData(data)
        }
    }
