...
import com.lifehopehealthapp.databinding.ActivityBloodGlucoseDetailpageBinding
...
class BloodGlucoseActivity : BaseActivity<BloodGlucoseViewModel, BloodGlucoseModel>() {
    ...
    private var mGlucoseID: ArrayList<Int> = ArrayList()
    private var mGlucoseType: ArrayList<String> = ArrayList()
    private var mGlucoseAvaible: ArrayList<Int> = ArrayList()

    private var mGlucoseList: ArrayList<GraphGlucoseList> = ArrayList<GraphGlucoseList>()
    private var adapter: BloodGlucoseAdapter? = null
    private var mValue: ArrayList<Bardatums> = ArrayList<Bardatums>()
    private var mDate: ArrayList<BarTimeStamps> = ArrayList<BarTimeStamps>()
    private var mColor: ArrayList<BarColorschemes> = ArrayList<BarColorschemes>()
    private var mListValue: ArrayList<DataListModel> = ArrayList<DataListModel>()

    private lateinit var binding: ActivityBloodGlucoseDetailpageBinding

    override fun getViewModel() = BloodGlucoseViewModel::class.java
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        binding = ActivityBloodGlucoseDetailpageBinding.inflate(layoutInflater)
        ...
        binding.recyclerViewListData.layoutManager =
        ...
        binding.tabLayout.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab) {
                ...
                (binding.tabLayout as TabLayout).setTabTextColors(
                    ContextCompat.getColor(this@BloodGlucoseActivity, R.color.white),
                    ContextCompat.getColor(this@BloodGlucoseActivity, R.color.purple_500)
                )
            }
        ...
        binding.buttonAddVitals.setOnClickListener {
            ...
            intent = Intent(this@BloodGlucoseActivity, AddVitalsActivity::class.java)
            intent.putExtra(Constants.VITALS_ID, vitalID)
            intent.putExtra(Constants.VITALS_NAME, name)
            intent.putExtra(Constants.VITALS_GLUCOSE_LIST, glucoseString.toString())
    ...
    private fun callEndDate(dateLimit: Int) {
        ...
        try {
            ...
            Log.e("shareStartDate", shareStartDate.toString())
            val tz = TimeZone.getDefault()
            val now = Date()
    ...
    private fun getGraphView() {
        if (Utils.isNetworkAvailable(this)) {
            ...
            if (lastDate!!.toInt() == 0) {
                ...
                viewModel.getBloodGlucoseData(token!!, aJsonObject)
                ...
                when (it) {
                    is Resource.Success -> {
                        ...
                        if (it.value.getStatusCode() == 200) {
                            ...
                            Log.e("mGlucoseAvaible", mGlucoseAvaible.size.toString())
                            mValue =
                                it.value.getValue()!!.data!!.getBarData() as ArrayList<Bardatums>
                            mDate =
                                it.value.getValue()!!.data!!.getBarTimestamp() as ArrayList<BarTimeStamps>
                            mColor = it.value.getValue()!!.data!!
                                .getBarColorSchemes() as ArrayList<BarColorschemes>
                            ...
                            if (mValue.size == 0) {
                                ...
                                binding.recyclerViewListData.adapter = null
                                ...
                                binding.recyclerViewListData.setHasFixedSize(true)
                                Log.e(
                                    "getBarDataList",
                                    it.value.getValue()!!.data!!.getBarDataList()!!.size.toString()
                                )
                                adapter =
                                    BloodGlucoseAdapter(
                                        it.value.getValue()!!.data!!.getBarDataList(),
                                        this
                                    )
                                binding.recyclerViewListData.adapter = adapter
                            }
    ...
    private fun initGraphValue(
        mValue: ArrayList<Bardatums>,
        mDate: ArrayList<BarTimeStamps>,
        mColor: ArrayList<BarColorschemes>
    ) {
        binding.bloodGlucoseChart.setDrawBarShadow(false)
        binding.bloodGlucoseChart.setDrawValueAboveBar(true)
        binding.bloodGlucoseChart.getDescription().setEnabled(false)
        binding.bloodGlucoseChart.setMaxVisibleValueCount(60)
        binding.bloodGlucoseChart.setPinchZoom(false)
        binding.bloodGlucoseChart.setDrawGridBackground(false)
        binding.bloodGlucoseChart.setFitBars(false)
        binding.bloodGlucoseChart.getViewPortHandler().setMaximumScaleX(2f)
        binding.bloodGlucoseChart.getViewPortHandler().setMaximumScaleY(1f)
        val trans: Transformer = binding.bloodGlucoseChart.getTransformer(YAxis.AxisDependency.LEFT)
        binding.bloodGlucoseChart.setXAxisRenderer(
            CustomXAxisRenderer(
                binding.bloodGlucoseChart.getViewPortHandler(),
                binding.bloodGlucoseChart.getXAxis(), trans
            )
        )
        binding.bloodGlucoseChart.setExtraBottomOffset(50f)

        val xAxis: XAxis = binding.bloodGlucoseChart.getXAxis()
        ...
        val leftAxis: YAxis = binding.bloodGlucoseChart.getAxisLeft()
        ...
        val rightAxis: YAxis = binding.bloodGlucoseChart.getAxisRight()
        rightAxis.isEnabled = false

        val l: Legend = binding.bloodGlucoseChart.getLegend()
        ...
        binding.bloodGlucoseChart.animateY(1000)
        binding.bloodGlucoseChart.invalidate()
    }

    //private fun setData(date: Int, range: Float) {
    private fun setData(
        range: ArrayList<Bardatums>,
        xAxis: XAxis,
        mDateValue: ArrayList<BarTimeStamps>,
        mColorValue: ArrayList<BarColorschemes>
    ) {
        ...
        val set1: BarDataSet
        if (binding.bloodGlucoseChart.getData() != null &&
            binding.bloodGlucoseChart.getData().getDataSetCount() > 0
        ) {
            set1 = binding.bloodGlucoseChart.getData().getDataSetByIndex(0) as BarDataSet
            set1.values = values
            binding.bloodGlucoseChart.getData().notifyDataChanged()
            binding.bloodGlucoseChart.notifyDataSetChanged()
        } else {
            ...
            data.barWidth = 0.5f
            binding.bloodGlucoseChart.setData(data)
        }
    }
    ...
    private fun apiCall() {
        if (Utils.isNetworkAvailable(this)) {
            ...
            if (binding.bloodGlucoseChart.data != null) {
                binding.bloodGlucoseChart.getData().clearValues()
                binding.bloodGlucoseChart.clear()
            }
    ...
    private fun ShareVitals() {
        if (Utils.isNetworkAvailable(this)) {
            if (shareStartDate == 0 || shareEndDate == 0) {
                ...
                var json: String? = ""
                json = gson.toJson(data)
                Log.e("ShareModel1---->", json.toString())
