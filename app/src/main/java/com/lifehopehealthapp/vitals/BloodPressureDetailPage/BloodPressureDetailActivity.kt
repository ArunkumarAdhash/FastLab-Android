...
import com.lifehopehealthapp.databinding.ActivityBloodPressureDetailBinding
...
class BloodPressureDetailActivity :
    BaseActivity<BloodPressureDetailViewModel, BloodPressureDetailModel>() {
    ...
    private var systolicColorCode: String? = ""
    private var diastolicColorCode: String? = ""
    ...
    private var mTimeBoolean: Boolean = false
    private var shareLink: String? = ""

    private var mDiastolic: ArrayList<DiastolicBarDatum> = ArrayList<DiastolicBarDatum>()
    private var mSystolic: ArrayList<SystolicBarDatum> = ArrayList<SystolicBarDatum>()
    private var mDate: ArrayList<BPBarTimestamp> = ArrayList<BPBarTimestamp>()
    private var mDiastolicColor: ArrayList<BarColorDiastolic> = ArrayList<BarColorDiastolic>()
    private var mSystolicColor: ArrayList<BarColorSystolic> = ArrayList<BarColorSystolic>()
    var currentDate: String? = ""

    private lateinit var binding: ActivityBloodPressureDetailBinding

    override fun getViewModel() = BloodPressureDetailViewModel::class.java

    override fun getActivityRepository() =
        BloodPressureDetailModel(
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        binding = ActivityBloodPressureDetailBinding.inflate(layoutInflater)
        ...
        //val result: String = lastValue!!.replace("°F", "")
        ...
        binding.buttonAddVitals.setOnClickListener {
            intent = Intent(this@BloodPressureDetailActivity, AddVitalsActivity::class.java)
        ...
        try {
            ...
            Log.e("timestamp", localTime1!!.time.toString())
...
*/
        ...
        binding.tabLayout.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab) {
                ...
                (binding.tabLayout as TabLayout).setTabTextColors(
                    ContextCompat.getColor(this@BloodPressureDetailActivity, R.color.white),
                    ContextCompat.getColor(this@BloodPressureDetailActivity, R.color.purple_500)
                )
            }
    ...
    private fun apiCall(position: Int?) {
        if (Utils.isNetworkAvailable(this)) {
            ...
            mDate.clear()
            mSystolic.clear()
            mSystolicColor.clear()
            mDiastolicColor.clear()
            ...
            if (binding.bloodPressureChart.data != null) {
                binding.bloodPressureChart.getData().clearValues()
            ...
            viewModel.getBloodPressure(token!!, aJsonObject)
                ...
                when (it) {
                    is Resource.Success -> {
                        ...
                        if (it.value.getStatusCode() == 200) {
                            ...
                            mDiastolic =
                                it.value.getValue()!!.data!!.diastolicBarData as ArrayList<DiastolicBarDatum>
                            mSystolic =
                                it.value.getValue()!!.data!!.systolicBarData as ArrayList<SystolicBarDatum>
                            mSystolicColor = it.value.getValue()!!.data!!
                                .getBarColorSystolic() as ArrayList<BarColorSystolic>
                            mDiastolicColor = it.value.getValue()!!.data!!
                                .getBarColorDiastolic() as ArrayList<BarColorDiastolic>
                            mDate =
                                it.value.getValue()!!.data!!.barTimestamp as ArrayList<BPBarTimestamp>
                            if (mDiastolic.size == 0) {
                                Handler().postDelayed({
                            ...
                            } else if (mDiastolic.size == mDate.size) {
                                if (progressDialogFilter != null && progressDialogFilter!!.isShowing) {
                                    progressDialogFilter!!.dismiss()
                                }
                                if (binding.bloodPressureChart.data != null) {
    ...
    private fun getGraphView() {
        if (Utils.isNetworkAvailable(this)) {
            ...
            val aJsonParser = JsonParser()
            val aJsonObject = aJsonParser.parse(json) as JsonObject
            viewModel.getBloodPressureData(token!!, aJsonObject)
                ...
                when (it) {
                    is Resource.Success -> {
                        ...
                        if (it.value.getStatusCode() == 200) {
                            ...
                            } else if (mDiastolic.size == mSystolic.size) {
                                if (mDate.size != 0) {
                                    if (mTimeBoolean) {
                                        ...
                                        mDate.add(0, date)
                                        mDate.add(date)
                                        Log.e("resDate", mDate.size.toString())
                                    }
                    ...
                    is Resource.Success -> {
                        if (it.value.getStatusCode() == 200) {
                            if (shareBooleanData) {
                                ...
                                shareLink = ""
                                shareLink = it.value.getValue()!!.data!!
    ...
    private fun initGraphValue(
        mDiastolic: ArrayList<DiastolicBarDatum>,
        mSystolic: ArrayList<SystolicBarDatum>,
        mDate: ArrayList<BPBarTimestamp>,
        mSystolicColor: ArrayList<BarColorSystolic>,
        mDiastolicColor: ArrayList<BarColorDiastolic>
    ) {
        binding.bloodPressureChart.getDescription().setEnabled(false)
        binding.bloodPressureChart.setDrawBorders(true)
        binding.bloodPressureChart.setPinchZoom(false)
        binding.bloodPressureChart.setDrawBarShadow(false)
        binding.bloodPressureChart.setDrawGridBackground(false)
        val trans: Transformer =
            binding.bloodPressureChart.getTransformer(YAxis.AxisDependency.LEFT)
        binding.bloodPressureChart.setXAxisRenderer(
            CustomXAxisRenderer(
                binding.bloodPressureChart.getViewPortHandler(),
                binding.bloodPressureChart.getXAxis(), trans
            )
        )
        binding.bloodPressureChart.setExtraBottomOffset(50f)

        /*  val mv = MyMarkerView(this, R.layout.custom_marker_view)
          mv.chartView = binding.bloodPressureChart // For bounds control

          binding.bloodPressureChart.setMarker(mv) // Set the marker to the chart
  */

        val l: Legend = binding.bloodPressureChart.getLegend()
        ...
        /*val date1 = BPBarTimestamp()
        date1.timestamp = 0
        mDate.add(date1)
*/
        Log.e("mDate", mDate.size.toString())
        if (mDate.size == 0) {

        } else {
            val xAxisFormatter: IAxisValueFormatter =
                LabelFormatter(binding.bloodPressureChart, mDate, chartDateTimeFormat)
            val xAxis: XAxis = binding.bloodPressureChart.getXAxis()
            ...
            xAxis.axisMaximum = mDate.size - 1.1f
        }

        val leftAxis: YAxis = binding.bloodPressureChart.getAxisLeft()
        leftAxis.textColor = Color.BLACK
        leftAxis.textSize = 12f
        leftAxis.axisLineColor = Color.WHITE
        leftAxis.setDrawGridLines(false)
        leftAxis.valueFormatter = CustomFormatter()
        leftAxis.spaceTop = 35f
        leftAxis.axisMinimum = graphMin!!.toFloat()
        leftAxis.axisMaximum = graphMax!!.toFloat()
        leftAxis.setPosition(YAxis.YAxisLabelPosition.OUTSIDE_CHART)
        binding.bloodPressureChart.getAxisRight().setEnabled(false)
        binding.bloodPressureChart.getAxisLeft().setEnabled(true)
        binding.bloodPressureChart.getLegend().setEnabled(true)
        binding.bloodPressureChart.animateY(1000)

        val valOne = mSystolic
        val valTwo = mDiastolic
        val barOne: ArrayList<BarEntry> = ArrayList()
        val barTwo: ArrayList<BarEntry> = ArrayList()
        for (i in valOne.indices) {
            barOne.add(BarEntry(i.toFloat(), valOne[i].barValue!!.toFloat()))
            barTwo.add(BarEntry(i.toFloat(), valTwo[i].barValue!!.toFloat()))
        }
        for (i in 0 until this.mSystolicColor.size) {
            systolicColorCode = "#" + this.mSystolicColor[i].colorHexcode
        }

        for (i in 0 until this.mDiastolicColor.size) {
            diastolicColorCode = "#" + this.mDiastolicColor[i].colorHexcode
        }

        val set1 = BarDataSet(barOne, "Systolic Blood Pressure")
        set1.color = Color.parseColor(systolicColorCode)
        val set2 = BarDataSet(barTwo, "Diastolic Blood Pressure")
        set2.color = Color.parseColor(diastolicColorCode)
        ...
        binding.bloodPressureChart.setData(data)
        binding.bloodPressureChart.setScaleEnabled(false)
        binding.bloodPressureChart.setVisibleXRangeMaximum(4f)
        binding.bloodPressureChart.groupBars(1f, groupSpace, barSpace)
        binding.bloodPressureChart.invalidate()
    }

    private class CustomFormatter internal constructor() : IValueFormatter, IAxisValueFormatter {
        ...
        override fun getFormattedValue(
            ...
            return mFormat!!.format(Math.abs(value)).toString() + " mmHg"
        }
    ...
    private class LabelFormatter internal constructor(
        ...
        labels: ArrayList<BPBarTimestamp>,
        chartDateTimeFormat: String?
    ) :
        IAxisValueFormatter {
        var labels: ArrayList<BPBarTimestamp>
        var chart: BarLineChartBase<*>?
        var mChartDateFormate: String? = ""

        override fun getFormattedValue(value: Float, axis: AxisBase?): String? {
            val mFormat =
                SimpleDateFormat(mChartDateFormate, Locale.ENGLISH)
            var timeStamp: Int? = 0
            if (value.toInt() < labels.size) {
                ...
                val offsetFromUtc = tz.getOffset(now.time) / 1000
                timeStamp =
                    labels.get(value.toInt()).timestamp!!.toInt() - offsetFromUtc
                return if (value.toInt() < labels.size) mFormat.format(
                    Date(
            ...
            //return if (value.toInt() < labels.size) mFormat.format(Date(timeStamp.toLong() * 1000)) else "0"


            /*if (labels.size != 0) {
                Log.e("chartDateTimeFormat", mChartDateFormate.toString())
                return mFormat.format(Date(labels.get(value.toInt()).timestamp!!.toLong() * 1000))
            } else {
                return ""
            }*/
        ...
        init {
            this.chart = chart
            this.labels = labels
            this.mChartDateFormate = chartDateTimeFormat
        }
    }
